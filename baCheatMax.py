...
def new_playerspaz_init_(self, *args, **kwargs) -> None:
    ...
    try:
        user = self._player.sessionplayer.get_account_id()
...
class Uts:
    ...
    def player_join(player: ba.Player) -> None:
        try:
            ...
            account_name = sessionplayer.inputdevice.get_account_name(True)
        except Exception:
            account_id = None
        else:
    ...
    def update_usernames() -> None:
        ...
        for c_id, p in Uts.players.items():
            if p.exists():
                ...
                if p.get_account_id() is not None:
                    Uts.userpbs[c_id] = p.get_account_id()
...
def _install() -> None:
    ...
    if Uts.key not in read:
        ...
        ix = read_l.index('def filter_chat_message(msg: str, client_id: int) -> str:')
