...
class Builder:
    ...
    def super_shield(self) -> None:
        if self._dead or not self.node.exists() or self.ex_super_shield is None:
    ...
    def effect_attraction(self, enemy: bs.Actor | None) -> None:
        if (enemy is not None and self.node.exists()
        and self.owner is not None and self.owner.exists()):
    ...
    def end_effect_attraction(self, enemy: bs.Node = None) -> None:
        if (enemy is not None and enemy.exists() and self.node.exists()
        and self.owner is not None and self.owner.exists()):
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        if isinstance(msg, _TouchMessage):
            ...
            if isinstance(msg, bomb.ExplodeMessage):
                if self.bomb_type != 'Xfactor' and not self.owner.exists():
                ...
                if self.bomb_type == 's.m.b':
                    ...
                    if self.node.exists() and not self.node.sticky:
...
class Tr784(bs.Actor):
    ...
    def shoot(self):
        ...
        if self.last_ball is not None:
            if not self.last_ball.node.exists():
                self.last_ball = None
            else: return
...
class HiperBox(bs.Actor):
    ...
    def anims(self) -> None:
        ...
        if anim == 'anim1':
            ...
            text = PopupText(i, self.node.position, scale=5.0).autoretain()
            ...
            x = bs.gettexture
            textures = [x('black'), x('aliBSRemoteIOSQR'), x('eggTex2'), x('aliColorMask'), self.node.color_texture]
...
def new_map(self, vr_overlay_offset: Optional[Sequence[float]] = None) -> None:
...
class ExPowerupsActivity(bs.Activity[bs.Player, bs.Team]):
    ...
    def _generated(self):
        ...
        self.nodes = [
            Text(getlanguage(des), (-380.0, 300.0), transition=Text.Transition.FADE_IN, h_align=Text.HAlign.LEFT).autoretain(),
            Image(data['tex'], (-430.0, 300.0), scale=(60.0, 60.0), mesh_opaque=bs.getmesh('image1x1')).autoretain(),
            Text(getlanguage(self.powtype), (-480.0, 300.0), color=(ex.get_random_color(x=1.5, mx=0.8) + (1.0, )), transition=Text.Transition.FADE_IN, h_align=Text.HAlign.RIGHT).autoretain(),
            
            Image(bs.gettexture('buttonPunch'), (-650.0, 200.0), scale=(70.0, 70.0), color=(1.0, 0.7, 0.0, 0.8)).autoretain(),
            Text(getlanguage('act.button.punch'), (-600.0, 200.0 - 15.0), color=(1.0, 1.0, 1.0, 0.8), h_align=Text.HAlign.LEFT).autoretain(),
            
            Image(bs.gettexture('buttonPickUp'), (-650.0, 100.0), scale=(70.0, 70.0), color=(0.2, 0.6, 1.0, 0.8)).autoretain(),
            Text(getlanguage('ow.switches'), (-600.0, 100.0 - 15.0), color=(1.0, 1.0, 1.0, 0.8), h_align=Text.HAlign.LEFT).autoretain(),
            
            Image(bs.gettexture('buttonBomb'), (-650.0, 0.0), scale=(70.0, 70.0), color=(1.0, 0.3, 0.3, 0.8)).autoretain(),
            Text(getlanguage('ow.changes'), (-600.0, 0.0 - 15.0), color=(1.0, 1.0, 1.0, 0.8), h_align=Text.HAlign.LEFT).autoretain(),
            
            Image(bs.gettexture('buttonJump'), (-650.0, -100.0), scale=(70.0, 70.0), color=(0.1, 1.0, 0.1, 0.8)).autoretain(),
            Text(getlanguage('act.button.jump'), (-600.0, -100.0 - 15.0), color=(1.0, 1.0, 1.0, 0.8), h_align=Text.HAlign.LEFT).autoretain(),
            ]
...
class BoxWindow(PopupWindow):
    def __init__(self, transition='in_right'):
        ...
        self._backButton = b = bui.buttonwidget(parent=self._root_widget,autoselect=True,
                                               ...
                                               button_type='back',on_activate_call=bs.Call(self._back))
    ...
    def _set_tab(self, tab, sound: bool = False):
        ...
        if tab == 'Action 1':
            ...
            GLOBAL['Tab'] = "Action 1"
            OptionsWindow(position=(0.0, 0.0), size=(800.0, 800.0))
            bui.containerwidget(edit=self._root_widget, transition='out_scale')
            return
...
class OptionsWindow(PopupWindow):
    def __init__(self, position, size, delegate: bui.Widget = None, **kwargs):
        bgc = (0.23, 0.22, 0.5)
        sc = 1.0 if bui.app.ui_v1.uiscale is bui.UIScale.SMALL else 0.6
        super().__init__(position, size, bg_color=bgc, scale=sc, **kwargs)
        ...
        self._scrollwidget = bui.scrollwidget(
            parent=self.root_widget,
            position=(position[0]+40, position[1]+60),
            highlight=False,
